...
2. **相機拍照** — 用裝置相機即拍即查
3. **語音查詢** — 說出關鍵字（例如料號「200866」），比對圖片標籤
...
├── 010315.jpg ... 722713.jpg          # 圖片資料庫（OAV 真實物料照片，現有 230 張，檔名即料號）
...
原本 15 張示範圖（A001_1.jpg ~ E003_1.jpg，螺絲/狗/OAV 機械/鋸片/汽車 5 類示範）已移除，改用 OAV 實際物料照片：**檔名（去除副檔名）即為料號**，例如 `200866.JPG` 對應料號 `200866`。目前共 230 張，用途是讓使用者用手機拍照後，比對出最相似的料號並判斷外觀是否相符。因此 `tags.json` 目前是空物件 `{}`——語音/文字查詢會依系統原本的 fallback 機制，直接用檔名（即料號）當關鍵字比對，不需要額外維護中文標籤。
...
- 相似度比對是「一張查詢圖 vs 資料庫全部圖片」線性掃描，圖片數量在數百張（現有 230 張）沒有效能問題；若圖片數量成長到數千張以上，需要改用近似最近鄰（ANN）索引。
